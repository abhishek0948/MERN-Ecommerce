...
const UploadProduct = ({ onClose,fetchAllProducts }) => {
  ...
  const handleDeleteProductImage = async (idx) => {
   ...
   const handleSubmit = async (e) => {
    ...
    if(!data.productName || !data.brandName || !data.price || !data.sellingPrice || !data.productImages || !data.description || !data.category) {
    ...
    <div className="fixed w-full h-full bg-slate-200 bg-opacity-60 top-0 left-0 right-0 bottom-0 flex justify-center items-center">
      <div className="p-4 bg-white w-full rounded max-w-2xl h-full max-h-[80%] overflow-hidden">
        ...
        <div className="flex justify-between items-center pb-4">
          <h2 className="font-medium text-lg">Upload Product</h2>
        ...
        <form className="grid p-4 gap-2 overflow-y-scroll h-full" onSubmit={handleSubmit}>
          ...
          <label htmlFor="productImage" className="mt-2">Product Images :</label>
          ...
          <div>
            {
              data.productImages[0] ? (
                <div className="flex items-center gap-2">
                  {
                    data.productImages.map((el,idx) => {
                      return (
                        <div className="relative group">
                          <img src={el} alt="el" width={80} height={80} className="bg-slate-100 border cursor-pointer hover:scale-105 transition-all"
                          onClick={() => {
                ...
                <p className="text-red-500 text-xs w-full flex justify-center">*please upload images</p>
              )
            }
          </div>
          ...
          <button type="submit" className="px-3 py-2 mb-10 bg-red-500 hover:bg-red-700 text-white rounded-md">Upload Product</button>
        </form>
      </div>

      {/* display full Product Image */}
      {openFullScreen && (
        <DisplayProductImage imgUrl={fullScreenImg} onClose={() => setOpenFullScreen(false)}/>
      )}
